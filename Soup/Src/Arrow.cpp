...
void Arrow::setArrow(Vec2f p, Vec2f m) {
  ...
  /* spine */
  const Vec2f v[4] = {{p - t}, {p}, {p + m - h}, {p + m}};

  /* points */
  const Vec2f pt[8] = {{v[0] - s}, {v[1] - s}, {v[2] - s}, {v[3] - s},
...
void Arrow::setTexture(sf::Texture &tex) { texture = &tex; }

void Arrow::setTailTexRec(Rectf r) {
  tlen = r.w / r.h;
  sfQuadTexcoords(verts[0], r);
}

void Arrow::setBodyTexRec(Rectf r) { sfQuadTexcoords(verts[1], r); }

void Arrow::setHeadTexRec(Rectf r) {
  hlen = r.w / r.h;
