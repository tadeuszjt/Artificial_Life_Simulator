...
GuiPanel::GuiPanel() { setTexture(Resources_Panel1()); }

void GuiPanel::setTexture(sf::Texture &tex) {
  texture = &tex;
  tileSize = tex.getSize().x / 3;
  for (int r = 0; r < 3; r++)
    for (int c = 0; c < 3; c++)
      sfQuadTexcoords(verts[r][c],
                      Rectf(tileSize, tileSize, c * tileSize, r * tileSize));

  setRect(rec);
}

void GuiPanel::setColour(Colour c) {
...
void GuiPanel::drawOn(sf::RenderTarget &target, sf::RenderStates states) const {
  states.texture = texture;
  target.draw((sf::Vertex *)verts, 3 * 3 * 4, sf::Quads, states);
}

void GuiPanel::setRect(Recti r) {
  rec = r;
...
Recti GuiPanel::rect() const { return rec; }
