...
bool Grid::inRange(Vec2f pos, float dist, void **ptrloc) {
...
bool Grid::nearest(Vec2f pos, void **ptrloc, float *distloc) {
  float dist2 = FLT_MAX;
  bool found = false;
  void *ptr;
  Tile *tp, **app = tileAt(pos)->area;
...
Rectf Grid::rect() const { return region; }

Gridder::Gridder(void *Ptr) : grouper(Ptr) {
  grouper.owner = this;
  grid = nullptr;
  tile = nullptr;
  read = nullptr;
...
void Gridder::setGrid(Grid &_grid) {
  grid = &_grid;
...
bool Gridder::pollLocals(void **ptrloc) {
  if (read) {
    for (;;) {
      ...
      if ((read = tile->area[areai++]) == nullptr)
        break;
      else
        readi = 0;
    }
