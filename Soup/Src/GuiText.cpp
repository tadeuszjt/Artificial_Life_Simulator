...
void GuiText::printf(const char *fmt, ...) {
  const int len = 100;
  char buf[len];
  va_list args;
  va_start(args, fmt);
...
void GuiText::alignLeft() {
  alignment = 0;
  setRect(rec);
}

void GuiText::alignCentre() {
  alignment = 1;
  setRect(rec);
}

void GuiText::alignRight() {
  alignment = 2;
  setRect(rec);
}

void GuiText::fontSans() { text.setFont(Resources_Sans()); }

void GuiText::fontSerif() { text.setFont(Resources_Serif()); }

void GuiText::fontMono() { text.setFont(Resources_Mono()); }

void GuiText::setColour(Colour c) {
  text.setFillColor(sf::Color(c.r, c.g, c.b, c.a));
}

void GuiText::drawOn(sf::RenderTarget &target, sf::RenderStates states) const {
  target.draw(text, states);
}

void GuiText::setRect(Recti r) {
  rec = r;
...
Recti GuiText::rect() const { return rec; }
