...
void SoupWindow::setMouseCB(void (*mouse)(Vec2i, bool, bool)) {
  mouseCB = mouse;
}

void SoupWindow::setKeyboardCB(void (*keyboard)(sf::Keyboard::Key, bool,
                                                bool)) {
  keyboardCB = keyboard;
}

void SoupWindow::setDrawCB(void (*draw)(sf::RenderTarget &)) { drawCB = draw; }

void SoupWindow::setDrawGuiCB(void (*draw)(sf::RenderTarget &)) {
  drawGuiCB = draw;
}

Vec2f SoupWindow::mouseWindow() const {
  sf::Vector2i m = sf::Mouse::getPosition(sfWindow);
  return Vec2f(m.x, m.y);
}

Vec2f SoupWindow::mouseWorld() const {
  sf::Vector2i m = sf::Mouse::getPosition(sfWindow);
