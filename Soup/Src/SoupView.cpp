...
void SoupView::resize(sf::Vector2u size) {
  viewsize.x = size.x;
  viewsize.y = size.y;
...
void SoupView::zoom(float delta, sf::Vector2i mousepos) {
  targetzoom -= delta * 0.2;
  mouse.x = mousepos.x - halfsize.x;
  mouse.y = mousepos.y - halfsize.y;
}

void SoupView::setZoom(float scale, sf::Vector2f centre) {
