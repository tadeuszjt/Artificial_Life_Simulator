...
void GuiButton::mouse(Vec2i pos, bool pressed, bool released) {
  if (pressed) {
    if (pos.inRect(rec))
      held = true;
  } else if (released) {
...
void GuiButton::drawOn(sf::RenderTarget &target,
                       sf::RenderStates states) const {
  if (held)
    pressedPanel.drawOn(target, states);
  else if (hovered)
    hoveredPanel.drawOn(target, states);
  else
    releasedPanel.drawOn(target, states);
}

void GuiButton::setRect(Recti r) {
  rec = r;
...
Recti GuiButton::rect() const { return rec; }
