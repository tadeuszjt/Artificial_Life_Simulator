...
GuiWindow::GuiWindow() {
  ...
  title.text.setOutlineThickness(0.8);
  title.text.setStyle(sf::Text::Bold);
  title.alignCentre();
}

void GuiWindow::mouse(Vec2i pos, bool press, bool release) {
...
void GuiWindow::drawOn(sf::RenderTarget &target,
...
void GuiWindow::setRect(Recti r) {
  rec = r;
  margin.setRect(rec);
  panel.setRect(rec);
...
Recti GuiWindow::rect() const { return rec; }
