...
void Icon::loadFile(const char filename[]) {
...
Iconer::~Iconer() { unSetIcon(); }

void Iconer::setIcon(Icon &ic) {
  unSetIcon();
  icon = &ic;
...
void Iconer::setIconRect(Rectf r) {
  iconRec = r;
  if (icon) {
...
void Iconer::setRect(Rectf r) {
  rec = r;
  setPos(pos);
}

void Iconer::setCol(Colour c) {
  col = c;
  if (icon) {
    sf::Vertex *quad = &icon->verts[index * 4];
    sfQuadColour(quad, c);
...
void Iconer::setPos(Vec2f p, Vec2f m) {
  pos = p;
  mag = m;
  setPos(pos);
}
