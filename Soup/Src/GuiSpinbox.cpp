...
void GuiSpinbox::setCount(int c) {
  if (c < min)
    c = min;
  else if (c > max)
    c = max;
...
void GuiSpinbox::setCountMultiplier(float m) { mult = m; }

void GuiSpinbox::setCountRange(int _min, int _max) {
  min = _min;
  max = _max;
  setCount(count);
}

float GuiSpinbox::value() const { return count * mult; }

void GuiSpinbox::mouse(Vec2i pos, bool pressed, bool released) {
...
void GuiSpinbox::drawOn(sf::RenderTarget &target,
...
void GuiSpinbox::setRect(Recti r) {
  rec = r;
  panel.setRect(rec);
...
Recti GuiSpinbox::rect() const { return rec; }
