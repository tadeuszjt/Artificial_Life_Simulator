...
NeuralNet::NeuralNet(int numInputs, int hiddenRows, int hiddenCols,
                     int numOutputs) {
  ni = numInputs;
  no = numOutputs;
  rows = hiddenRows;
...
NeuralNet::~NeuralNet() { delete[] fltmem; }

void NeuralNet::randomise() {
  for (int i = 0; i < nw; i++)
    weight[i] = randflt(-1.0, 1.0);
}

float NeuralNet::sigmoid(float a) const {
  const float a4 = a * 4;
...
void NeuralNet::draw(sf::RenderTarget &target) {
