...
static void mutateNet(NeuralNet &net) {
...
static Colour netColour(NeuralNet &net) {
  double sum[3] = {0., 0., 0.};
  for (int i = 0; i < net.nw; i++)
...
Boater::Boater() : mode(this), gridder(this), net(13, 24, 3, 2) {
  iconOut.setCol(Colour(68, 68, 68));
}

void Boater::setPos(Vec2f p, Vec2f d) {
  pos = p;
  dir = d.normal();
  gridder.setPos(pos);
...
void Boater::setCol(Colour c) {
  col = c;
  iconBack.setCol(c);
}

void Boater::update() {
  float sensors[13] = {8., 8., 8., 8., 8., 8., 8., 8., 8., 8., 8., 8., 8.};
...
void Boats::mouse(Vec2f pos, bool press, bool release) {
  mPos = pos;
  if (press) {
    held = true;
...
void Boats::draw(sf::RenderTarget &target) {
  if (selected.count) {
    Boater *s = selected[0];
...
void Boats::threadUpdate(int i0, int i1) {
  for (int i = i0; i < i1; i++) {
    Boater *b = list[i];
    b->update();
  }
