...
void Blobber::setCol(Colour c) {
  col = c;
  iconFill.setCol(c);
}

Gobbler::Gobbler() : grouper(this), gridder(nullptr) {
  icon.setRect(Rectf(3, 3, -1.5, -1.5));
  dir = Vec2f_randNorm();
  vel = dir * 0.4;
}

void Gobbler::setPos(Vec2f p) {
  pos = p;
  gridder.setPos(p);
  icon.setPos(p);
}
...
Blobber *Ecosystem::spawnBlobber() {
...
Gobbler *Ecosystem::spawnGobbler() {
...
void Ecosystem::drawOn(sf::RenderTarget &target) {
